"""
---------------Spatial Correlation Capture---------------------

Description:
    1. This script trains a dual-scale transformer model for generating unsupervised tract-level embeddings from per-building features. 
    2. It implements self-supervised contrastive learning with data augmentation on building sequences within each tract.
...
Required input files: building_features.npy, building_rot_type.npz are generated by data preprocessing pipeline (from resnet.py) and combined_building_list.csv is generated by raterization.py

For Feature Ablation Study:
To perform feature ablation (e.g., removing building type), modify the feature construction step by excluding the building type one-hot vectors from 'combined_features'. 
Repeat the full pipeline for all years/cities as needed.
